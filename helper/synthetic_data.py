...
def generate_categories(amounts) -> list:
    """."""    
    categories = []        
    for category, category_per_price in CATEGORY_PER_PRICE.items():
        percentage, min_price, max_price = category_per_price
        n = int(TOTAL_UNIQUE_TRANSACTIONS * percentage)
        for _ in range(n):
...
def update_transactions(transactions: list, chains: list) -> list:
    ...
    for key, chain in chains.items():
        ...
        for i, idx in enumerate(chain):
            ...
            original_transaction['category'] = [category for category, category_per_price in CATEGORY_PER_PRICE.items() if int(inject_amounts[i]) in range(int(category_per_price[1]), int(category_per_price[2]))][0]
